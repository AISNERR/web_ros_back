...
import apps.sci_pop.tests.test_sci_pop_admin
from apps.sci_pop.models import SciPop, SciPopModeratorComments
...
class ApiSciPopTestCase(BaseSciPopAPITestCase):
    ...
    def test_sci_pop_delete(self):
        ...
        with self.assertRaises(SciPop.DoesNotExist):
            SciPop.objects.get(id=self.post.id)
    ...
    def test_sci_pop_archiving(self):
        ...
        self.post.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.post.sci_pop_status.status, "archived")
