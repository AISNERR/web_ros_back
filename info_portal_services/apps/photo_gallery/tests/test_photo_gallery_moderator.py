...
class ApiGalleryTestCase(BaseGalleryAPITestCase):
    ...
    def test_gallery_list(self):
        self.post = PostInGallery.objects.get(pk=107)
    ...
    def test_gallery_update(self):
        self.post = PostInGallery.objects.get(pk=101)
        url = reverse("photo-gallery-rud", args=(101,))
        post_coordinates = [
            round(uniform(0, 60), 6),
            round(uniform(0, 60), 6)
        ]
    ...
    def test_gallery_delete(self):
        self.post = PostInGallery.objects.get(pk=108)
        url = reverse("photo-gallery-rud", args=(self.post.id,))
    ...
    def test_gallery_moderate_set_status(self):
        ...
        self.post.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.post.photo_gallery_status.id, data.get("photo_gallery_status"))
        url = reverse("photo-gallery-info", args=(self.post.id,))
        response = self.client.get(url, format="json")
        self.assertContains(response, text=data.get("comment"))
        self.assertEqual(self.post.photo_gallery_comments.all()[0].comment, data.get("comment"))
    ...
    def testing_add_comments(self):
        self.post = PostInGallery.objects.get(pk=105)
    ...
    def test_get_managing_gallery(self):
        ...
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.json()), PostInGallery.objects.count())
